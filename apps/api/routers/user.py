    ...
    try:
        if user_id != "demo":
            ...
            user_rows = await execute_query(
                "SELECT email, full_name, tier, created_at FROM users WHERE id = ?", (user_id,)
            )
            if user_rows:
                export_data["user_info"] = {
                    "email": user_rows[0][0],
                    "full_name": user_rows[0][1],
                    "tier": user_rows[0][2],
                    "created_at": user_rows[0][3],
                }

            # Get usage history
            usage_rows = await execute_query(
                "SELECT usage_date, generations_count FROM daily_usage WHERE user_id = ?", (user_id,)
            )
            export_data["usage_stats"] = [{"date": r[0], "count": r[1]} for r in usage_rows]
    except Exception as e:
        logger.error("export.failed", user_id=user_id, error=str(e))

    requested_at = datetime.datetime.now(datetime.UTC).replace(microsecond=0).isoformat()
    expires_at = (datetime.datetime.now(datetime.UTC) + datetime.timedelta(hours=24)).replace(microsecond=0).isoformat()
...
async def delete_account(request: DeleteAccountRequest, http_request: Request):
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=403, detail="Invalid password — account deletion requires re-authentication"
            )
