...
def _mock_team_result():
    ...
    return {
        ...
        "reasoning": {"budget_agent": "mock", "differential_agent": "mock", "risk_agent": "mock"},
    }
...
def test_team_output_valid_passes():
    ...
    valid_team = {
        "players": [{"id": f"p{i}", "role": roles_mix[i], "team": "MI" if i < 5 else "CSK"} for i in range(11)],
        "captain": "p0",
        "vice_captain": "p1",
        "total_cost": 99,
        "predicted_total": 500,
...
def test_match_id_validator():
    ...
    with pytest.raises(ValueError):
        _validate_match_id("   ")
    with pytest.raises(ValueError):
        _validate_match_id("match id")
    with pytest.raises(ValueError):
        _validate_match_id("'; DROP TABLE--")
    with pytest.raises(ValueError):
        _validate_match_id("../../../etc/passwd")
...
def test_firewall_ip_tracking():
    ...
    from security.ai_firewall import _ip_violations, _is_ip_banned, _record_violation
...
def test_auth_token_revocation():
    ...
    from middleware.auth import _revoked_tokens, is_token_revoked, revoke_token
...
def test_auth_service_email_validation():
    ...
    with pytest.raises(ValidationError):
        _validate_email("@no-local.com")
    with pytest.raises(ValidationError):
        _validate_email("a" * 255 + "@x.com")
